...
const MainPage = () => {
  ...
  const handleApprove = () => {
    addToast('pending', 'Approving SNOW tokens...');
    setTimeout(() => {
      setIsApproved(true);
      addToast('success', 'SNOW tokens approved');
    }, 3000);
  };
  const handleStake = () => {
    if (!stakeAmount || parseFloat(stakeAmount) <= 0) return;
    addToast('pending', 'Staking SNOW tokens...');
    setTimeout(() => {
      addToast('success', `Successfully staked ${stakeAmount} SNOW`);
      setStakeAmount('');
    }, 4000);
  };
        ...
        <Grid container spacing={4} >
          ...
          <Grid item xs={12} md={4} >
            <Card sx={{ bgcolor: '#111827', color: '#E2E8F0', borderRadius: '12px' }}>
              <CardContent>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  <Typography variant="body2" sx={{ color: '#94A3B8' }}>Your SNOWv3 Balance</Typography>
                  <IconButton sx={{ color: '#7DC4FF' }}>
                    <RefreshCw size={16} />
                  </IconButton>
                </Box>
                <Typography variant="h5" sx={{ fontWeight: 600 }}>{snowBalance}</Typography>
                <Typography variant="body2" sx={{ color: '#94A3B8' }}>SNOW</Typography>
              </CardContent>
            </Card>
            ...
            <Card sx={{ bgcolor: '#111827', mt: 2, borderRadius: '12px', opacity: 0.8 }}>
              <CardContent>
                ...
                <Typography variant="body2" sx={{ color: '#94A3B8' }}>SNOWv2</Typography>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid item xs={12} md={8}>
            {currentPhase === 'stake' && isWalletConnected && (
              <Card sx={{ bgcolor: '#111827', color: '#E2E8F0', borderRadius: '12px' }}>
                <CardContent>
                  <Typography variant="h6" mb={2} sx={{ fontWeight: 600 }}>Stake SNOW Tokens</Typography>
                  <TextField
                    type="number"
                    fullWidth
                    value={stakeAmount}
